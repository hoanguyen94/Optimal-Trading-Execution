...
K = 100       # replay memory capacity: number of episodes
THETA = 2      # prioritization parameter for replay, 1 for uniformly random replay, > 1 for prioritized replay
iteration = 1  # number of times to replay

# uncomment it if you already define stock to be trained in Training.py
stock = ['CATY']

def replay(d, c, dictState):
    """ Update replay sample
    :param d: deviation of Q values
    :param c: order
    :param dictState: dictionary of states
    :return:
            """
    # save probability
    p = d[max(0, c - K):(c + 1)] ** THETA
    order = range((c + 1)-(max(0, c - K)))
    ind = random.choices(order, p)[0]
    epi = epiList[max(0, c - K):(c + 1)][ind]
    d[max(0, c - K) + ind] = run_episode(q_func, q_func2, df[df['epi'] == epi], dictState, for_training=True, c=c)
    return None

def tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState, v, terminal, c=0):
    ...
    # uncomment it if want to plot convergence of Q-estimates
    q_func2[currentState + actionInd + (c,)] = q_func[currentState + actionInd]
    return abs(d)



def run_episode(q_func, q_func2, df, dictState, for_training=False, c=0):
    """ Runs one episode """
    ...
    accReward, PI = 0, 0
    nextState = (len(marketState) + 2) * [None]  # 2 private variables
    vMarket, vLimit = 0, 0
    vState = None
    vTest = V
    dSub = []
    ...
    if for_training:
        for time in np.arange(noStep):  # remaining time
            for v in np.arange(lot, V + lot, lot):  # remaining shares > 0
                ...
                if time != 0:
                    ...
                    for action in actionSpace:
                        for actType in ['limit', 'market']:
                            if actType == 'market':
                                ...
                                # update q value
                                if vMarket > 0:
                                    dSub.append(tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState,
                                                       vMarket, terminal=False, c=c))
                                else:
                                    dSub.append(tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState,
                                                       vMarket, terminal=True, c=c))
                                ...
                                if vLimit > 0:
                                    dSub.append(tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState,
                                                       vLimit, terminal=False, c=c))
                                else:
                                    dSub.append(tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState,
                                                       vLimit, terminal=True, c=c))
                    ...
                    # update q value
                    dSub.append(tabular_q_learning(q_func, q_func2, currentState, actionInd, reward, PI, nextState, vMarket,
                                       terminal=True, c=c))
        ...
        for time in np.arange(noStep):
            ...
            currentState = tuple(np.append(currentState, [noStep - 1 - time, vState]))
            ...
            # set current action
            if np.array(q)[~np.isnan(q)].size > 0:   #if the state is visited
                currentAct = actionList[np.nanargmax(q)]
            elif time == noStep-1 and vTest > 0 and np.isnan(q).all() == True:   # if the state is not visited and the end of episode
                currentAct = (0, vTest)
            ...
            if currentAct[0] == 0:  # market
                reward = (df['bestBidPrice'].iloc[time] - initialPrice + PI) * min(currentAct[1],
                                                                                   df['bestBidVol'].iloc[time])
                ...
                PI = df[['const', 'OFICoeff', 'TICoeff', 'OICoeff']].iloc[noStep - 1 - time] @ np.array(
                    [1, sharesFilled - currentAct[1], 0, sharesFilled - currentAct[1]]) if currentAct[
                                                                                                          1] > 0 else 0
                vTest -= sharesFilled
...
if __name__ == '__main__':
    for s in stock:
        ...
        inputFileFmt = inputFolder + '\\' + s + '\\*BestPrice2.csv'
        ...
        print(df2[['tradingVol', 'tradingImb', 'movingDiff', 'spread', 'OFI']].describe())
        ...
        # market variables
        rangec = ['tradingVol', 'tradingImb', 'movingDiff', 'spread', 'OFI', 'aveMD', 'orderImb']
        ...
        rangeq = {'tradingVol': [-np.inf, 1, 100, np.inf], 'tradingImb': [-np.inf, 0, 100, np.inf],
        ...
        # for all actions
        # df2 = df[df['optimalA1'] > 0]  # keep only positive actions
        df2 = df
        X = sm.add_constant(
        ...
        # initialize probability for replay
        d = np.zeros(trainSize)
        ...
        for a in [0.1]:
            ...
            # set it to 0 if dont want to plot convergence of Q-estimates
            ...
            for i in tqdm(epiList[:trainSize]):
                if df[df['epi'] == i].shape[0] > 1:
                    d[c] = run_episode(q_func, q_func2, df[df['epi'] == i], dictState, for_training=True, c=c)

                    # save probability
                    for j in range(iteration):
                        replay(d, c, dictState)
                    c += 1

            for i in tqdm(epiList[trainSize:]):
                if df[df['epi'] == i].shape[0] > 1:
                    optimalQ, optimalAType, optimalA, accReward = run_episode(q_func, q_func2, df[df['epi'] == i], dictState,
                                                                              for_training=False)
            ...
            # store as csv files
            outputFileh5 = outputFolder + "\\" + "BestPrice_Replay" + str(THETA) + str(a) + str(BETA)  + ".h5"
            h5f = h5py.File(outputFileh5, 'w')
            h5f.create_dataset(s, data=q_func2)
            h5f.close()

        # store as csv files
        outputFile = outputFolder + "\\" + str(BETA) + "BestPrice_Replay" + str(THETA) + ".csv"
